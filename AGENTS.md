...
- **Database**: SQLite (`racks.db`) storing company configuration, rack metadata, 42U unit schemas, checklists, and asset URLs/Base64 data.
...
- **Persistence**: All state modifications (company branding, rack edits, 42U units, checklists) persist via `/api/data` POST.
- **Rack Structure**: 42U units, front/rear photos (9:16 container aspect ratio), QR codes, technician notes, and verification checklists (cabling, power, temperature, labeling, grounding).
