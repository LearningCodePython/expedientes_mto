...
- **Database**: SQLite (`racks.db`) for company branding, rack metadata, 42U unit schemas, checklists, and assets.
...
- **Persistence**: All state changes (company config, rack edits, 42U slots, checklists) persist via `/api/data` POST.
- **Rack Structure**: 42U units, front/rear photos, QR codes, technician notes, and verification checklists (cabling, power, temperature, labeling, grounding).
